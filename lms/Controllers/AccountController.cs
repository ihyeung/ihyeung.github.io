...
namespace LMS.Controllers
{
    ...
    public class AccountController : CommonController
    {
        ...
        /// <summary>
        /// Helper function that generates an auto-incremented unique uID 
        /// for every new user account created.
        /// </summary>
        /// <returns></returns>
        public string GenerateUniqueUid()
        {
            var next = Convert.ToUInt32(GetMostRecentlyGeneratedUid().Substring(1)) + 1;
            Debug.Assert(next <= 9999999);
            return "u" + next;
        }

        /// <summary>
        /// Retrieves the most recently generated uID from the database.
        /// </summary>
        /// <returns></returns>
        public string GetMostRecentlyGeneratedUid()
        {
            var max_student = db.Student.Max(x => x.UId);
            var max_prof = db.Professor.Max(x => x.UId);
            var max_admin = db.Administrator.Max(x => x.UId);
            System.Diagnostics.Debug.WriteLine("max stduent uid: " + max_student);
            System.Diagnostics.Debug.WriteLine("max professor uid: " + max_prof);
            System.Diagnostics.Debug.WriteLine("max admin uid: " + max_admin);
            string[] uids = new string[] { max_student, max_prof, max_admin };
            Array.Sort(uids); //Sorts in ascending order, last element will have 
            return uids[2];
        }
        ...
        public string CreateNewUser(string fName, string lName, DateTime DOB, string SubjectAbbrev, string role)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine("{0} {1} Birthdate: {2}", s.FirstName, s.LastName, s.birth_date);
            }
            if (role.Equals("Administrator"))
            {
                db.Administrator.Add(new Administrator(uid, fName, lName, DOB));
            }
            else if (role.Equals("Student"))
            {
                db.Student.Add(new Student(uid, fName, lName, DOB, SubjectAbbrev));
            }
            else
            {
                Debug.Assert(role.Equals("Professor"));
                db.Professor.Add(new Professor(uid, fName, lName, DOB, SubjectAbbrev));
            }
            db.SaveChanges();
